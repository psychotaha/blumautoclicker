...
if __name__ == "__main__":
    ...
    print("gozinei ke id telegrame shoma ro namayesh dade entekhab konid (telegram ra ghabl az ejraye barname baz konid va varede robate blum shavid): ")
    for i, (title, hwnd) in enumerate(windows):
        print(f"{i + 1}: {title}")

    choice = int(input("gozineye morede nazar ra vared konid: ")) - 1
    ...
    while True:
        try:
            target_percentage = input("baraye random shodane shanse entekhabe setareha , meghdari ra beyne 0 va 1 vared konid ke 1 yani tamame setare ha gerefte shavad . gozineye pishnehadi beyne 0.4 va 0.6 mibashad ke beyne 140-150 setare gerefte mishavad vali shoma mitavanid ta 0.9 in ragham ra bala bebarid.adade morede nazar ra vared konid : ")
    ...
    while True:
        try:
            collect_freeze = int(input("setarehaye yakhi ham click beshan (optional) ?? 1 - YES, 2 - NO: "))
    ...
    logger.log('bad az start zadane bazi , rooye F6 dar keyboarde khod click konid')
